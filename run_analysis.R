...
# Quick name adjustment for three "V1"s
original_var_names <- names(train_df)
...
feature_indices <- c(mean_indices, std_indices)
V_indices <- feature_indices + 2 # added y and subject to left of test
full_indices <- c(1,2, V_indices)
full_indices_sort <- sort(full_indices)

# Extract measurements
measure_df <- full_df[,full_indices_sort]

## ((3)) NAME ACTIVITIES

# Make new activity labels
activity_labels_txt <- read.table("./UCI_HAR_Dataset/activity_labels.txt")
activity_labels_orig <- activity_labels_txt[,2]
activity_labels_lc <- tolower(activity_labels_orig)
activity_labels <- sub("_", " ", activity_labels_lc)

# Extract the original class labels
data_class_labels <- measure_df[,2] # numeric factors

# Convert the numeric to character factors
data_activities <- sapply(data_class_labels, function(x) activity_labels[x]) 

# Replace the current "classlabel" column with "activities" factor
...
avg_df_init <- tibble::as_tibble(measure_df) # make a data frame table
avg_df <- avg_df_init %>%
